...
export function ReelCard({ project, index }: { project: Project; index: number }) {
  ...
  const inner = (
    <div className="grid grid-cols-12 items-baseline gap-4 py-7 md:gap-8 md:py-10">
      ...
      <div className="col-span-10 md:col-span-7">
        ...
        <h3 className="ink-italic mt-2 font-display text-display-lg text-balance leading-[0.95] transition-colors duration-500 ease-out group-hover:text-accent">
          {project.title}
        </h3>
        <p className="mt-3 max-w-[60ch] text-sm text-ink-muted md:text-base">
          {project.summary}
        </p>
      </div>
  ...
  return (
    <motion.li
      ...
      className="group relative border-t border-line transition-colors duration-500 last:border-b hover:bg-bg-raised/50"
    >
      {project.external ? (
        <a
          href={project.href}
        ...
        <Link href={project.href} data-cursor="view" className="block px-2 focus:outline-none">
          {inner}
        </Link>
      )}
    </motion.li>
